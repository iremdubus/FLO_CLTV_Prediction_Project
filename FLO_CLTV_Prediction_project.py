...
# 2. Aykırı değerleri baskılamak için gerekli olan outlier_thresholds
...
# Zaman farkını nanosecond (ns) cinsinden alıp, günlere (10^9 * 60 * 60 * 24) ve haftalara (7) çevirelim
cltv_df["recency_cltv_weekly"] = ((df["last_order_date"] - df["first_order_date"]).dt.total_seconds()) / (60 * 60 * 24 * 7)

# Aynı şekilde analiz tarihi ile ilk sipariş tarihi arasındaki farkı hesaplıyoruz
cltv_df["T_weekly"] = ((analysis_date - df["first_order_date"]).dt.total_seconds()) / (60 * 60 * 24 * 7)

# Sipariş sayısını ve ortalama parasal değeri hesaplıyoruz
...
inconsistent_records = cltv_df[(cltv_df['frequency'] == 0) & (cltv_df['recency_cltv_weekly'] > 0)]
...
# ve grup isimlerini veri setine ekleyiniz. cltv_segment ismi ile atayınız.
